...
def toka(n, primes):
    ...
    for i in range(alku, n):
        ...
        if i % tulostusvali == 0:
            b = timer()
            tulostusvali = valiAika(b-a, tulostusvali)
        ...
        if prime:
            ...
            if lkm % talletusvali == 0:
                ...
                kirjoita(primes, "alkuluvut.txt", lkm,
                         lkm-ed_lkm, edAika)  # välitalletus
                talletusvali = valiAika(hh, talletusvali)
                ed_lkm = lkm
                edAika = timer()
            # yield(i)
    kirjoita(primes, "alkuluvut.txt", lkm-ed_lkm, edAika)
...
def kirjoita(primes, tiedosto, lkm, ero=0, edAika=0):  # "alkuluvut.txt"
    ...
    print(aika(edAika,timer()), ero)  # lkm = len(primes)
